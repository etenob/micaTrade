...
3.  **Whale Alert (Opcional):** Actualmente Alien exige ver ballenas. Un ajuste agresivo sería eliminar este requisito en `strategies.py`, permitiendo que Alien dispare solo con fuerza técnica (75%) si el resto de los filtros (EMA 55, ADX, Macro) son positivos.
...
| **Ajustar Rebote Suelo** | Alta 📈 | Alto 🔥 | Comprarás "cuchillos cayendo" con más frecuencia. |

---

## 🚫 LA PERILLA PROHIBIDA: Ignorar Señal de Fondo (LONG/SHORT)
Esta es la modificación **más arriesgada** que se puede hacer en el bot. Actualmente, Alien, Ciega y Gatillo **exigen** que el mercado sea `LONG` para comprar. 

**¿Qué significa "Bajar el Seguro"?**
Si quitamos el check de `sig['signal'] == 'LONG'` en `strategies.py`, el bot comprará monedas que están en caída libre, intentando adivinar el suelo.

*   **Riesgo:** Máximo 🔥 (Peligro de "Cuchillo Cayendo").
*   **Resultado:** Muchos más trades, pero con un Win Rate mucho más bajo. Verás muchas operaciones saltando por Stop Loss porque estás operando contra la marea del mercado.

**Recomendación:** Solo tocar esto en momentos de capitulación extrema del mercado (donde todo cae un 30% y se espera un rebote vertical), pero nunca dejarlo activado en un mercado normal.

> [!NOTE]
> **LA EXCEPCIÓN:** La única estrategia que hoy tiene "permiso" para dudar de la señal de fondo es **REBOTE SUELO**, ya que por su diseño matemático busca precisamente los giros cuando el mercado está sangrando.
> [!TIP]
