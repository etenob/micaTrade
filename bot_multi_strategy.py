    ...
    while True:
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                # Mapear cada símbolo a su función de análisis
                future_to_symbol = {executor.submit(JaimeMerinoIndicatorsEngine.get_real_trading_analysis, s): s for s in symbols}
            ...
            for s in symbols:
                ...
                triggered = StrategyEngine.get_active_signals(analysis)
                    ...
                    if not tm.get_active_trade(s):
                        if (total_exposure + cfg['order_amount']) <= Config.MAX_USDT_TOTAL_EXPOSURE:
                            ...
                            if Config.IS_LIVE_TRADING:
                                ...
                                # 🔔 Notificación Telegram: APERTURA
                                try:
                                    send_telegram_message(f"🚀 [{s}] ENTRADA DETECTADA\nEstrategia: {strat_name}\nPrecio aprox: ${analysis['current_price']}")
                                except: pass
...
def strategy_analyzer():
    symbol = request.args.get('symbol', Config.TRADING_SYMBOLS[0] if Config.TRADING_SYMBOLS else 'BTCUSDT').upper()
    
    # Datos en tiempo real o del cache
    data = latest_analysis.get(symbol)
    if not data:
        data = JaimeMerinoIndicatorsEngine.get_real_trading_analysis(symbol)
    
    # Reporte Multi-TF bajo demanda (incluye 15m mediante get_multi_tf_report)
    multi_tf = JaimeMerinoIndicatorsEngine.get_multi_tf_report(symbol)

    # Evaluar estado de todas las estrategias usando el motor oficial
    from strategies import StrategyEngine
    strategies_report = StrategyEngine.get_strategy_report(data)
...
def dashboard():
    ...
    active_trades = {t['symbol']: t for t in tm.get_all_active()}
    
    # Ordenar por confianza descendente
    sorted_data = dict(sorted(display_data.items(), key=lambda x: x[1]['signal']['signal_strength'], reverse=True))
...
def active_positions():
    ...
    for t in active_trades:
        ...
        t['new_signals'] = []
        if analysis:
            current_signals = StrategyEngine.get_active_signals(analysis)
            ...
            for upd in updates:
                ...
                if cmd in ["/start", "/help", "/ayuda"]:
                    ...
                    if len(cmd_parts) > 1:
                        ...
                        try:
                            from merino_math import JaimeMerinoIndicatorsEngine
                            multi_tf = JaimeMerinoIndicatorsEngine.get_multi_tf_report(coin)
                    ...
                    if analysis:
                        from strategies import StrategyEngine
                        active_sigs = StrategyEngine.get_active_signals(analysis)
