...
class TradeManager:
    ...
    def close_trade(symbol, exit_price, reason="Exit"):
        """Cierra un trade, calcula PnL y lo mueve al historial."""
        active = TradeManager._read_json('active_trades.json')
        trade_to_close = next((t for t in active if t['symbol'] == symbol), None)
        
        if trade_to_close:
            # Eliminar de activos
            new_active = [t for t in active if t['symbol'] != symbol]
            TradeManager._write_json('active_trades.json', new_active)
            ...
            history = TradeManager._read_json('trade_history.json')
            ...
            TradeManager._write_json('trade_history.json', history)
            return True
        return False

    @staticmethod
    def get_history():
        return TradeManager._read_json('trade_history.json')

    @staticmethod
    def get_all_active():
        return TradeManager._read_json('active_trades.json')
